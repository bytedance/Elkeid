...
* 如果测试机器kernel版本不在支持列表中，请自行编译ko文件和深层sign文件，文件需要放置在nginx对应服务器的：`/elkeid/nginx/ElkeidAgent/agent/plugin/driver/ko`下，ko/sign文件的格式应该遵循：`hids_driver_1.7.0.4_{uname -r}_{arch}.ko/sign` 放置完成后Agent会自动拉取对应的ko文件进行安装
