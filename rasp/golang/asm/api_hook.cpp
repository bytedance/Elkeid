...
int hookAPI(void *address, void *replace, void **backup) {
    ...
    while (true) {
        ...
        if ((instruction.mnemonic == ZYDIS_MNEMONIC_SUB || instruction.mnemonic == ZYDIS_MNEMONIC_ADD) &&
    ...
    return trap_hook((std::byte *) address + offset, replace, backup);
}
