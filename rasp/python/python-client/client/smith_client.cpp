...
constexpr auto INBOX_MAX_SIZE = 1000;
...
void CSmithClient::onBufferRead(bufferevent *bev) {
    ...
    {
        AutoMutex _0_(mInboxMutex);

        if (mInbox.size() <= INBOX_MAX_SIZE)
            mInbox.emplace_back(buffer);
    }
...
void CSmithClient::onTimer() {
    if (mBev) {
        ...
        char heartbeat[1024] = {};
        char format[] = R"({"pid": %d, "runtime": "%s", "runtime_version": "%s", "time": %ld, "message_type": %d, "probe_version": "%s", "data": {}})";

        snprintf(
                heartbeat, sizeof(heartbeat), format,
                mPID, RUNTIME, mVersion.c_str(),
                CTimeHelper::getUnixTimestamp(), HEARTBEAT, PROBE_VERSION
                );

        writeBuffer(heartbeat);
    } else {
        LOG_INFO("reconnect");
        connect();
    }
...
void CSmithClient::loopThread() {
    ...
    pthread_setname_np(pthread_self(), "python-client");
    event_base_dispatch(mEventBase);
}

bool CSmithClient::fetch(std::string &message) {
    AutoMutex _0_(mInboxMutex);

    if (mInbox.empty()) {
        LOG_INFO("inbox empty");
        return false;
    }
