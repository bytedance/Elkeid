...
/*
 * HIDS driver types (translated from xfer/ring.h)
 */
...
pub const AL_EBPF_ARGV: c_int = 0xAA;
pub const AL_EBPF_EXE: c_int = 0xAE;
...
extern "C" {
    ...
    /*
     * access control related interfaces (both kmod and ebpf supported)
     */

     pub fn ac_init(ring_type: c_int, arg_control: *const u8) -> c_int;
     pub fn ac_fini(ring_type: c_int) -> c_int;
    ...
    /*
     * unified interfaces for LKM & ebpf (for future)
     */
    pub fn tb_init(ring_type: c_int, arg_control: *const u8) -> c_int;
    pub fn tb_fini(ring_type: c_int) -> c_int;
    pub fn tb_read(
         msg: *const u8,
         len: c_int,
    ...
    /*
     * LKM related interfaces
     */

    pub fn tb_init_kmod(ring_type: c_int, arg_control: *const u8) -> c_int;
    pub fn tb_fini_kmod(ring_type: c_int) -> c_int;
    ...
    /* access control for kmod */
    pub fn ac_init_kmod(ring_type: c_int, arg_control: *const u8) -> c_int;
    pub fn ac_fini_kmod(ring_type: c_int) -> c_int;

    /* 设置规则，支持list或json格式 */
    pub fn ac_setup_kmod(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /* 清除特定类型的所有规则 */
    pub fn ac_clear_kmod(ac: c_int) -> c_int;

    /* 检测规则生效与否，仅适用于allowlist */
    pub fn ac_check_kmod(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /* 删除特定规则，仅适用于allowlist */
    pub fn ac_erase_kmod(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /* 读取当前所有规则，目前仅适用于allowlist */
    pub fn ac_query_kmod(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /*
     * ebpf related interfaces
     */

    /* 加载 ebpf程序 */
    pub fn tb_load_ebpf(path: *const u8) -> c_int;
    /* 卸载 ebpf程序 */
    pub fn tb_unload_ebpf();

    pub fn tb_init_ebpf(ring_type: c_int, arg_control: *const u8) -> c_int;
    pub fn tb_fini_ebpf(ring_type: c_int) -> c_int;
    pub fn tb_read_ebpf(
         msg: *const u8,
         len: c_int,
         cb: extern "C" fn(*const AtomicBool) -> c_int,
    ...
    pub fn ac_init_ebpf(ring_type: c_int, arg_control: *const u8) -> c_int;
    pub fn ac_fini_ebpf(ring_type: c_int) -> c_int;

    /* access control for ebpf */

    /* 设置规则，支持list或json格式 */
    pub fn ac_setup_ebpf(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /* 清除特定类型的所有规则 */
    pub fn ac_clear_ebpf(ac: c_int) -> c_int;

    /* 检测规则生效与否，仅适用于allowlist */
    pub fn ac_check_ebpf(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /* 删除特定规则，仅适用于allowlist */
    pub fn ac_erase_ebpf(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    /* 读取当前所有规则，目前仅适用于allowlist */
    pub fn ac_query_ebpf(ac: c_int, ptr: *const u8, len: c_int) -> c_int;

    pub fn tb_query_ebpf(version: *const u8, sz: *mut usize) -> c_int;

    /* kernel panic 避让相关 */
    pub fn safeboot_check() -> c_int; /* 返回1表示不用避让，非1则需要避让 */
    pub fn safeboot_clear() -> c_int; /* 返回0表示成功，其它则为错误码 */
}
...
pub const ENABLE_SELF_PROTECTION: &[u8] = b"Y";
pub const DISABLE_SELF_PROTECTION: &[u8] = b"N";
