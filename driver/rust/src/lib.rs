...
mod bindings;
use bindings::RING_KMOD;
// use bindings::RING_KMOD_V1_7;
// use bindings::RING_KMOD_V1_8;
// use bindings::RING_KMOD_V1_9;
...
/*
 *  v1.7:  RingSlot::new(RING_KMOD_V1_7, "/proc/elkeid-endpoint")
 *  v1.8:  RingSlot::new(RING_KMOD_V1_8, "/proc/elkeid-endpoint")
 *  v1.9:  RingSlot::new(RING_KMOD_V1_9, "/sys/module/elkeid/parameters/control_trace")
 */
impl RingSlot {
    pub fn new(ring_dev: c_int, dev_path: &str) -> Result<(Self, impl FnOnce()), i32> {
        let control = Arc::new(AtomicBool::new(false));
        let dev_path_cstr = CString::new(dev_path.as_bytes()).unwrap();
        let rc = unsafe { tb_init_kmod(ring_dev, dev_path_cstr.as_ptr() as _) };
        if rc == 0 {
            Ok((
                Self {
...
/*
 *  v1.7:  SmithControl::new(RING_KMOD_V1_7, "/proc/elkeid-endpoint")
 *  v1.8:  SmithControl::new(RING_KMOD_V1_8, "/proc/elkeid-endpoint")
 *  v1.9:  SmithControl::new(RING_KMOD_V1_9, "/sys/module/elkeid/parameters/control_trace")
 */
impl SmithControl {
    pub fn new(ring_dev: c_int, ctrl_path: &str) -> Result<Self> {
        unsafe {
            let ctrl_path_cstr = CString::new(ctrl_path.as_bytes()).unwrap();
            let rc = ac_init(ring_dev, ctrl_path_cstr.as_ptr() as _);
            if rc != 0 {
                return Err(anyhow!(
                    "ac_init({:x},{}) error:{}",
                    RING_KMOD,
    ...
    pub fn self_protection_enable(&self) -> Result<()> {
        return control_flag(
            "/sys/module/elkeid/parameters/protector_switch",
...
impl EBPFConsumer {
    pub fn new(_dev_path: &str) -> Result<(Self, impl FnOnce()), i32> {
        let control = Arc::new(AtomicBool::new(false));
        let rc = unsafe { tb_init_ebpf(RING_EBPF, ptr::null()) };
        if rc == 0 {
            Ok((
                Self {
                    control: control.clone(),
...
impl EBPFLoader {
    pub fn new(path: &str) -> Result<(Self, impl FnOnce()), i32> {
        let control = Arc::new(AtomicBool::new(false));
        let path_cstr = CString::new(path.as_bytes()).unwrap();
        let rc = unsafe { tb_load_ebpf(path_cstr.as_ptr() as _) };
        if rc == 0 {
            Ok((
                Self {
                    control: control.clone(),
...
impl EBPFControl {
    pub fn new(ctrl_path: &str) -> Result<Self> {
        unsafe {
            let ctrl_path_cstr = CString::new(ctrl_path.as_bytes()).unwrap();
            let rc = ac_init(RING_EBPF, ctrl_path_cstr.as_ptr() as _);
            if rc != 0 {
                return Err(anyhow!(
                    "ac_init({:x},{}) error:{}",
                    RING_EBPF,
    ...
    pub fn get_version(ver_buf: &mut [u8]) -> Result<(), i32> {
        if ver_buf.len() < 16 {
            return Err(0);
        }
        let rc = unsafe {
            tb_query_ebpf(
                ver_buf.as_mut_ptr() as _,
                ptr::null_mut()
            )
        };
        if rc != 0 {
    ...
    pub fn clear_all(&self) -> Result<()> {
        unsafe {
            let mut rc = ac_clear(AL_EBPF_ARGV);
            if rc != 0 {
                return Err(anyhow!("ac_clear({:x}) error:{}", AL_EBPF_ARGV, rc));
            }

            rc = ac_clear(AL_EBPF_EXE);
            if rc != 0 {
                return Err(anyhow!("ac_clear({:x}) error:{}", AL_EBPF_EXE, rc));
            }
        }
        return Ok(());
    }
    ...
    pub fn ac_add_allow_exe_bytes(&self, target: &[u8]) -> Result<()> {
        unsafe {
            let cstr = CString::new(target)?;
            let rc = ac_setup(AL_EBPF_EXE, cstr.as_ptr() as _, target.len() as _);
            if rc < 0 {
                return Err(anyhow!(
                    "ac_setup({:x},{:?},{}) error:{}",
                    AL_TYPE_EXE,
    ...
    pub fn ac_add_allow_argv_bytes(&self, target: &[u8]) -> Result<()> {
        unsafe {
            let cstr = CString::new(target)?;
            let rc = ac_setup(AL_EBPF_ARGV, cstr.as_ptr() as _, target.len() as _);
            if rc < 0 {
                return Err(anyhow!(
                    "ac_setup({:x},{:?},{}) error:{}",
                    AL_TYPE_ARGV,
    ...
    pub fn ac_check_allow_exe(&self, target: &str) -> Result<i32> {
        unsafe {
            let target_cstr = CString::new(target.as_bytes())?;
            let rc = ac_check(AL_EBPF_EXE, target_cstr.as_ptr() as _, target.len() as _);
            return Ok(rc);
        }
    }
    ...
    pub fn ac_check_allow_argv(&self, target: &str) -> Result<i32> {
        unsafe {
            let target_cstr = CString::new(target.as_bytes())?;
            let rc = ac_check(AL_EBPF_ARGV, target_cstr.as_ptr() as _, target.len() as _);
            return Ok(rc);
        }
    }
    ...
    pub fn ac_del_allow_exe_bytes(&self, target: &[u8]) -> Result<()> {
        unsafe {
            let target_cstr = CString::new(target)?;
            let rc = ac_erase(AL_EBPF_EXE, target_cstr.as_ptr() as _, target.len() as _);
            if rc < 0 {
                return Err(anyhow!(
                    "ac_erase({:x},{:?}) error:{}",
                    AL_EBPF_EXE,
    ...
    pub fn ac_del_allow_argv_bytes(&self, target: &[u8]) -> Result<()> {
        unsafe {
            let target_cstr = CString::new(target)?;
            let rc = ac_erase(AL_EBPF_ARGV, target_cstr.as_ptr() as _, target.len() as _);
            if rc < 0 {
                return Err(anyhow!(
                    "ac_erase({:x},{:?}) error:{}",
                    AL_EBPF_ARGV,
...
pub fn try_safeboot_check() -> i32 {
    unsafe { return safeboot_check(); }
}

pub fn try_safeboot_clear() -> i32 {
    unsafe { return safeboot_clear(); }
}
