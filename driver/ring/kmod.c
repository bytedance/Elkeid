...
static struct sd_event_format *g_sd_formats;
static struct sd_item_ent *   *g_sd_events;

static uint32_t g_types_mappings_v1_9[] = {
    /* U8/S8 */
    [1] = 1,
    [0x41] = 0x41,
    ...
    /* STRING */
    [8] = 9,    /* v3: XIDS and IPU added as 8 and 9 */
};

static int sd_init_events_v1_9(struct sd_event_format *fmt)
{
    int i, n;
    ...
    for (n = sizeof(*fmt), i = 0; i < fmt->nids; i++) {
        ...
        if (e[0].eid != i + 1) {
            printf("invalid item eid: %d\n", i);
            return -1;
        }
        if (e[0].size <= sizeof(*e) * 2) {
            printf("invalid item size: %d\n", i);
            return -2;
        }
        g_sd_events[i] = e;
        DEBUG(printf("format id: %d\n", i));
        DEBUG(sd_hexdump(e, e[0].size));
        n += e[0].size;
        if (n > fmt->size)
            break;
    }
...
static int sd_init_format_v1_9(int fd)
{
    ...
    if (sd_init_events_v1_9(g_sd_formats)) {
        free(g_sd_formats);
        g_sd_formats = NULL;
        g_sd_events = NULL;
        return -5;
...
static struct sd_item_ent *sd_query_event_v1_9(void *data, int *rec)
{
    ...
    if (eid > 0 && eid <= g_sd_formats->nids) {
        ...
        if (eid != item[0].eid || head[0].size < item[1].meta ||
            ...
            printf("eid: %u eid != item[0].eid: %u head[0].size < item[1].meta: %u head[1].meta != item[1].meta: %u head[1].xid != item[1].xid: %u\n", 
                    eid, eid != item[0].eid, head[0].size < item[1].meta,
                    head[1].meta != item[1].meta, head[1].xid != item[1].xid);
            return NULL;
        }
        *rec = head[0].size;
        ...
        printf("invalid eid: %u [1 - %u]\n", eid, g_sd_formats->nids);
    }
...
static int tb_init_format_v1_9(int fd)
{
...
static int tb_init_sd_events_v1_9()
{
    struct sd_item_ent *e;
    int i, j;

    for (i = 0; i < g_sd_formats->nids; i++) {
        e = g_sd_events[i];
        if (!e || !e->size || !e->eid)
            break;
        for (j = 2; j * sizeof(*e) < e[0].size; j++) {
            uint32_t val = e[j].item;
            if (!val)
                break;
            if ((val & 0xFF) == 8) // g_types_mappings_v1_9[]
                e[j].item = (val & 0xFFFFFF00) | 9UL;
        }
    }
    return 0;
}

static int tb_init_kmod_v1_9(int type, char *trace)
{
    char fds[64], rev[16];
    FILE *fp;
    int fd = -1, rc = 0;
    ...
    if (!trace)
        return -1;
    if (RING_KMOD != type && RING_KMOD_V1_9 != type)
        return -2;
    ...
    if (g_tb_trace.fd) {
        ...
        rc = tb_init_format_v1_9(g_tb_trace.fd - 1);
        if (rc)
            return rc;
        tb_init_sd_events_v1_9();
        goto opened;
    }

    fp = fopen(trace, "rb");
    if (!fp) {
        printf("Error: kernel module elkeid isn't loaded.\n");
        return -3;
    }
...
static int tb_fini_kmod_v1_9(int type)
{
 ...
 *
 * -EBADFD: b_init_kmod不成功，或者 /proc/elkeid-endpoint的fd被关闭
 * -ENOENT: 没有找到对应日志的格式信息，比如ringbuf的数据损坏
 * -EPROTO: 日志长度 与 对应的格式描述 不符合，数据损坏的可能
 *    ... : 系统函数read()返回-1时的错误码，已处理成负数
 */
static int tb_read_kmod_v1_9(char *msg, int len, int (*cb)(int *), int *ctx)
{
    ...
    do {
        ...
        if (tb->msgsz < 0)
            rc = -errno;
        DEBUG(printf("got new message: %d bytes\n", tb->msgsz));
        DEBUG(sd_hexdump(tb->pool, tb->msgsz));
    } while (rc < len && rc >= 0);
...
static int tb_is_passed_v1_9(struct timeval *tv, long cycle)
{
    struct timeval now;
...
static int tb_stat_kmod_v1_9(struct ring_stat *stat)
{
    struct tb_stat ts = {0};
...
static void tb_show_kmod_v1_9(struct ring_stat *s, struct ring_stat *l, struct ring_stat *n)
{
...
static int tb_register_binfmt_v1_9(void)
{
    int cmd = TRACE_IOCTL_FILTER + REGISTER_BINFMT;
    return ioctl(g_tb_trace.fd - 1, cmd, NULL);
}

static int tb_unregister_binfmt_v1_9(void)
{
    int cmd = TRACE_IOCTL_FILTER + UNREGISTER_BINFMT;
    return ioctl(g_tb_trace.fd - 1, cmd, NULL);
}

static int tb_pre_unload_v1_9(void)
{
    return tb_unregister_binfmt_v1_9();
}

static int ac_init_kmod_v1_9(int type, char *trace)
{
    char fds[64], rev[16];
    FILE *fp;
    int fd = -1, rc = 0;
    ...
    if (!trace)
        return -1;
    if (RING_KMOD != type && RING_KMOD_V1_8 != type)
        return -2;
...
static int ac_fini_kmod_v1_9(int type)
{
    if (RING_KMOD != RING_TYPE(type))
        return -1;
...
static int ac_setup_blocklist_v1_9(int ac, char *json, int len)
{
    zval *response, *rules;
    int i, rc = -1;
...
static int ac_setup_allowlist_v1_9(int ac, char *item, int len)
{
    int cmd;
...
static int ac_setup_kmod_v1_9(int ac, char *item, int len)
{
    if (!item)
        return -1;

    if (ac >= AL_TYPE_ARGV && ac <= AL_TYPE_PSAD)
        return ac_setup_allowlist_v1_9(ac, item, len);
    if (ac >= BL_JSON_DNS && ac <= BL_JSON_MD5)
        return ac_setup_blocklist_v1_9(ac, item, len);
    return -2;
}

static int ac_erase_allowlist_v1_9(int ac, char *item, int len)
{
    int cmd;
...
static int ac_erase_kmod_v1_9(int ac, char *item, int len)
{
    if (ac >= AL_TYPE_ARGV && ac <= AL_TYPE_PSAD)
        return ac_erase_allowlist_v1_9(ac, item, len);
    return -2;
}

static int ac_clear_allowlist_v1_9(int ac)
{
    int cmd;
...
static int ac_clear_blocklist_v1_9(int ac)
{
    int cmd, rc = -2;
...
static int ac_clear_kmod_v1_9(int ac)
{
    if (ac >= AL_TYPE_ARGV && ac <= AL_TYPE_PSAD)
        return ac_clear_allowlist_v1_9(ac);
    if (ac >= BL_JSON_DNS && ac <= BL_JSON_MD5)
        return ac_clear_blocklist_v1_9(ac);
    return -2;
}

static int ac_check_kmod_v1_9(int ac, char *item, int len)
{
    int cmd;
...
static int ac_query_kmod_v1_9(int ac, char *buf, int len)
{
    int cmd;
...
static int ac_process_kmod_v1_9(char *control, char *ptr, int len, int quiet)
{
    char *arg = ptr;
    int cmd, i, rc;

    rc = ac_init_kmod_v1_9(RING_KMOD, control);
    if (rc)
        return rc;
    ...
    switch(cmd) {
        case ADD_EXECVE_EXE_ALLOWLIST:
             ac_setup_kmod_v1_9(AL_TYPE_EXE, arg, strlen(arg));
             break;
        case EXECVE_EXE_CHECK:
             rc = ac_check_kmod_v1_9(AL_TYPE_EXE, arg, strlen(arg));
             if (quiet)
                break;
             if (rc == 0 || rc == 1)
        ...
        case DEL_EXECVE_EXE_ALLOWLIST:
             ac_erase_kmod_v1_9(AL_TYPE_EXE, arg, strlen(arg));
             break;
        case DEL_ALL_EXECVE_EXE_ALLOWLIST:
             ac_erase_kmod_v1_9(AL_TYPE_EXE, NULL, 0);
             break;

        case ADD_EXECVE_ARGV_ALLOWLIST:
            ac_setup_kmod_v1_9(AL_TYPE_ARGV, arg, strlen(arg));
            break;
        case EXECVE_ARGV_CHECK:
            rc = ac_check_kmod_v1_9(AL_TYPE_ARGV, arg, strlen(arg));
            if (quiet)
                break;
            if (rc == 0 || rc == 1)
        ...
        case DEL_EXECVE_ARGV_ALLOWLIST:
            ac_erase_kmod_v1_9(AL_TYPE_ARGV, arg, strlen(arg));
            break;
        case DEL_ALL_EXECVE_ARGV_ALLOWLIST:
            ac_erase_kmod_v1_9(AL_TYPE_ARGV, NULL, 0);
            break;
        ...
        case PRINT_EXE_ALLOWLIST:
            if (len < 1024) {
                if (cmd == PRINT_ARGV_ALLOWLIST)
                    rc = ac_query_kmod_v1_9(AL_TYPE_ARGV, NULL, 0);
                else
                    rc = ac_query_kmod_v1_9(AL_TYPE_EXE, NULL, 0);
            } else {
                memset(ptr, 0, len);
                if (cmd == PRINT_ARGV_ALLOWLIST)
                    rc = ac_query_kmod_v1_9(AL_TYPE_ARGV, ptr, len);
                else
                    rc = ac_query_kmod_v1_9(AL_TYPE_EXE, ptr, len);
                if (quiet)
                    break;
                for (i = 0; i < rc; i++) {
                    if (ptr[i])
    ...
    ac_fini_kmod_v1_9(RING_KMOD);
    return rc;
}

struct tb_ring_operations g_ring_v1_9 = {
    type: RING_KMOD_V1_9,
    version: 0x1091,

    ring_init: tb_init_kmod_v1_9,
    ring_fini: tb_fini_kmod_v1_9,

    ring_read: tb_read_kmod_v1_9,
    ring_is_passed: tb_is_passed_v1_9,
    ring_stat: tb_stat_kmod_v1_9,
    ring_show: tb_show_kmod_v1_9,

    register_binfmt: tb_register_binfmt_v1_9,
    unregister_binfmt: tb_unregister_binfmt_v1_9,
    pre_unload: tb_pre_unload_v1_9,


    ac_init: ac_init_kmod_v1_9,
    ac_fini: ac_fini_kmod_v1_9,

    ac_setup: ac_setup_kmod_v1_9,
    ac_erase: ac_erase_kmod_v1_9,
    ac_clear_allowlist: ac_clear_allowlist_v1_9,
    ac_clear_blocklist: ac_clear_blocklist_v1_9,
    ac_clear: ac_clear_kmod_v1_9,

    ac_check: ac_check_kmod_v1_9,
    ac_query: ac_query_kmod_v1_9,
    ac_process: ac_process_kmod_v1_9,
};

/*
 * trace-buffer support routines for v1.7
 */

static struct tb_trace_v1_7 {
    int fd;
    int instances; /* allowlist + ringbuf consuming */
} g_tb_trace_v1_7;


static int tb_init_kmod_v1_7(int type, char *trace)
{
    int fd = -1, rc = 0;
    ...
    if (!trace)
        return -1;
    if (RING_KMOD != type && RING_KMOD_V1_7 != type)
        return -2;

    /* consuming channel was already opened */
    if (g_tb_trace_v1_7.fd)
        goto opened;
    ...
    g_tb_trace_v1_7.fd = fd + 1;

opened:

    g_tb_trace_v1_7.instances++;
    return 0;
}

static int tb_fini_kmod_v1_7(int type)
{
    int fd = g_tb_trace_v1_7.fd - 1;

    if (RING_TYPE(type) != RING_KMOD)
        return 0;
    if (g_tb_trace_v1_7.fd <= 0)
        return 0;

    if (--g_tb_trace_v1_7.instances > 0)
        return g_tb_trace_v1_7.instances;
    g_tb_trace_v1_7.fd = 0;
    close(fd);
    return 0;
}
 ...
 *    ... : 系统函数read()返回-1时的错误码，已处理成负数
 */
static int tb_read_kmod_v1_7(char *msg, int len, int (*cb)(int *), int *ctx)
{
    int fd = g_tb_trace_v1_7.fd - 1;
...
static int tb_is_passed_v1_7(struct timeval *tv, long cycle)
{
    struct timeval now;
...
static int tb_stat_kmod_v1_7(struct ring_stat *stat)
{
    return 0;
}

static void tb_show_kmod_v1_7(struct ring_stat *s, struct ring_stat *l, struct ring_stat *n)
{
}

static int tb_register_binfmt_v1_7(void)
{
    return 0;
}

static int tb_unregister_binfmt_v1_7(void)
{
    return 0;
}

static int tb_pre_unload_v1_7(void)
{
    return tb_unregister_binfmt_v1_7();
}

static int ac_init_kmod_v1_7(int type, char *trace)
{
    int fd = -1, rc = 0;
...
static int ac_fini_kmod_v1_7(int type)
{
    if (RING_KMOD != RING_TYPE(type))
        return -1;
    ...
    if (--g_tb_trace_v1_7.instances > 0)
        return g_tb_trace_v1_7.instances;
    close(g_tb_trace_v1_7.fd - 1);
    g_tb_trace_v1_7.fd = 0;
    return 0;
}

static int ac_setup_allowlist_v1_7(int ac, char *item, int len)
{
    char dat[256];
    char *buf;
    int size, rc;

    if (!item || !len ||  g_tb_trace_v1_7.fd <= 0)
        return -1;
    if (ac != AL_TYPE_ARGV && ac != AL_TYPE_EXE)
        return -2;
    ...
    if (!buf)
        return -3;
    memset(buf, 0, size + 2);
    buf[0] = ac == AL_TYPE_ARGV ? 109 : 89;
    memcpy(&buf[1], item, size > len ? len : size);
    rc = write( g_tb_trace_v1_7.fd - 1, buf, (size < 4) ? 6 : (size + 2));
    if (buf && buf != dat)
        free(buf);
    return rc;
}

static int ac_setup_kmod_v1_7(int ac, char *item, int len)
{
    if (!item || !len ||  g_tb_trace_v1_7.fd <= 0)
        return -1;

    if (ac >= AL_TYPE_ARGV && ac <= AL_TYPE_PSAD)
        return ac_setup_allowlist_v1_7(ac, item, len);
    return -2;
}

static int ac_erase_allowlist_v1_7(int ac, char *item, int len)
{
    ...
    memset(buf, 0, size + 2);
    if (item)
        buf[0] = (ac == AL_TYPE_ARGV) ? 74 : 70;
    else
        buf[0] = (ac == AL_TYPE_ARGV) ? 119 : 117;
...
static int ac_erase_kmod_v1_7(int ac, char *item, int len)
{
    if (ac >= AL_TYPE_ARGV && ac <= AL_TYPE_PSAD)
        return ac_erase_allowlist_v1_7(ac, item, len);
    return -2;
}

static int ac_clear_allowlist_v1_7(int ac)
{
    return ac_erase_kmod_v1_7(ac, NULL, 0);
}

static int ac_clear_blocklist_v1_7(int ac)
{
    return -2;
}

int ac_clear_kmod_v1_7(int ac)
{
    if (ac >= AL_TYPE_ARGV && ac <= AL_TYPE_PSAD)
        return ac_clear_allowlist_v1_7(ac);
    if (ac >= BL_JSON_DNS && ac <= BL_JSON_MD5)
        return ac_clear_blocklist_v1_7(ac);
    return -2;
}

static int ac_check_kmod_v1_7(int ac, char *item, int len)
{
    char dat[256];
    char *buf;
    int size, rc;
...
static int ac_query_kmod_v1_7(int ac, char *buf, int len)
{
    return 0;
}

static int ac_process_kmod_v1_7(char *control, char *ptr, int len, int quiet)
{
    char *arg = ptr;
    int cmd, i = len, rc;

    rc = ac_init_kmod_v1_7(RING_KMOD, control);
    if (rc)
        return rc;
    ...
    rc = write(g_tb_trace_v1_7.fd - 1, arg, i);
    ac_fini_kmod_v1_7(RING_KMOD);
    return rc;
}


struct tb_ring_operations g_ring_v1_7 = {
    type: RING_KMOD_V1_7,
    version: 0x1070,

    ring_init: tb_init_kmod_v1_7,
    ring_fini: tb_fini_kmod_v1_7,

    ring_read: tb_read_kmod_v1_7,
    ring_is_passed: tb_is_passed_v1_7,
    ring_stat: tb_stat_kmod_v1_7,
    ring_show: tb_show_kmod_v1_7,

    register_binfmt: tb_register_binfmt_v1_7,
    unregister_binfmt: tb_unregister_binfmt_v1_7,
    pre_unload: tb_pre_unload_v1_7,


    ac_init: ac_init_kmod_v1_7,
    ac_fini: ac_fini_kmod_v1_7,

    ac_setup: ac_setup_kmod_v1_7,
    ac_erase: ac_erase_kmod_v1_7,
    ac_clear_allowlist: ac_clear_allowlist_v1_7,
    ac_clear_blocklist: ac_clear_blocklist_v1_7,
    ac_clear: ac_clear_kmod_v1_7,

    ac_check: ac_check_kmod_v1_7,
    ac_query: ac_query_kmod_v1_7,
    ac_process: ac_process_kmod_v1_7,
};


static uint32_t g_types_mappings_v1_8 [] = {
    /* U8/S8 */
    [1] = 1,
    [0x41] = 0x41,
    ...
    /* STRING */
    [7] = 9,    /* v3: XIDS and IPU added as 8 and 9 */
};

static int sd_init_events_v1_8(struct sd_event_format *fmt)
{
    int i, n, j;
    ...
    for (n = sizeof(*fmt), i = 0; i < fmt->nids; i++) {
        ...
        for (j = 2; j * sizeof(*e) < e[0].size; j++) {
            ...
            if (!val)
                break;
            if ((val & 0xFF) == 7)
                e[j].item = (val & 0xFFFFFF00) | 9UL; /* g_types_mappings[val & 0xFF] */
        }
        n += e[0].size;
        if (n > fmt->size)
            break;
...
static int sd_init_format_v1_8(int fd)
{
    struct sd_event_format fmt = {0};
    int rc;
    ...
    DEBUG(printf("got events format from driver: %d bytes.\n", rc));
    DEBUG(sd_hexdump(g_sd_formats, rc));
    if (sd_init_events_v1_8(g_sd_formats)) {
        free(g_sd_formats);
        g_sd_formats = NULL;
        g_sd_events = NULL;
        return -5;
    }
    DEBUG(printf("formats of v1.8 translated to v1.9:\n"));
    DEBUG(sd_hexdump(g_sd_formats, rc));

    return 0;
}

static int tb_init_format_v1_8(int fd)
{
    g_tb_trace.bufsz = TB_BUFFER_SIZE;
    g_tb_trace.pool = malloc(g_tb_trace.bufsz);
    if (!g_tb_trace.pool)
...
static int tb_init_kmod_v1_8(int type, char *trace)
{
    int fd = -1, rc = 0;
    ...
    if (g_tb_trace.fd) {
        ...
        /* try to initialize formats */
        rc = tb_init_format_v1_8(g_tb_trace.fd - 1);
        if (rc) {
            printf("Error: failed to init formats: %d\n", rc);
            return rc;
        }
        goto opened;
    }

    fd = open(trace, O_RDONLY);
    if (fd < 0) {
        printf("Error: kernel module smith isn't loaded: %s.\n", trace);
        return -3;
    }

    rc = tb_init_format_v1_8(fd);
    if (rc) {
        printf("Error: failed to init formats: %d\n", rc);
        close(fd);
        return rc;
...
static int tb_read_kmod_v1_8(char *msg, int len, int (*cb)(int *), int *ctx)
{
    struct tb_trace *tb = &g_tb_trace;
    int rc = 0, fd =tb->fd - 1;
    ...
    do {
        ...
        while (tb->start + sizeof(uint64_t) + 2 * sizeof(struct sd_item_ent) < tb->msgsz) {
            ...
            ret = sd_unpack(&msg[rc], len - rc, &dat[tb->start],
                            &rec, 0, sd_query_event_v1_9);
            /* len - rc: isn't long enough for new event */
            if (rc && !ret)
                break;

            /* v1.8: msgsz doesn't have timestamp included */
            rec += sizeof(uint64_t);
            /* got error in unpacking or msg is too small */
            DEBUG(printf("original message at %d: record: %xh ret: %d\n", tb->start, rec, ret));
            DEBUG(sd_hexdump(&dat[tb->start], rec));
...
static int ac_init_kmod_v1_8(int type, char *trace)
{
    int fd = -1, rc = 0;
...
struct tb_ring_operations g_ring_v1_8 = {
    type: RING_KMOD_V1_8,
    version: 0x1081,

    ring_init: tb_init_kmod_v1_8,
    ring_fini: tb_fini_kmod_v1_9,

    ring_read: tb_read_kmod_v1_8,
    ring_is_passed: tb_is_passed_v1_9,
    ring_stat: tb_stat_kmod_v1_9,
    ring_show: tb_show_kmod_v1_9,
