...
#define SD_TYPE_ENTRY_IPU(n, v) {{SD_TYPE_IPU}, {4}}
...
#define SD_TYPE_POINTER_IPU     SD_TYPE_ENTRY_IPU
...
{
	return (void *)&container_of(inode, struct socket_alloc, vfs_inode)->socket;
}
...
# if LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0)
    int prot;
    offset_of(struct sock, sk_padding);
    prot = READ_KERN(sk, ...);
...
static int sock_family(struct sock *sk)
{
    return READ_KERN(sk, __sk_common.skc_family);
}

static int query_ipv4(struct sock *sk,
                      struct ipaddr_ud *sip4,
                      struct ipaddr_ud *dip4)
{
    struct inet_sock *inet = (void *)sk;
    __be32 *sip = (void *)&sip4->v4.v4_addr32;
    __be32 *dip = (void *)&dip4->v4.v4_addr32;
    int port;
...
static int query_ipv6(struct sock *sk,
                      struct ipaddr_ud *sip6,
                      struct ipaddr_ud *dip6)
{
    struct inet_sock *inet = (void *)sk;
    struct in6_addr *sip = (void *)&sip6->v6;
    struct in6_addr *dip = (void *)&dip6->v6;
    ...
    /* read sip & sport */
    *sip = READ_KERN(sk, __sk_common.skc_v6_rcv_saddr);
    if (ipv6_addr_any(sip) && inet6)
        *sip = READ_KERN(inet6, saddr);
    port = READ_KERN(inet, inet_sport);
    sip6->port = swap16(port);

    /* read dip & dport */
    *dip = READ_KERN(sk, __sk_common.skc_v6_daddr);
    port = READ_KERN(sk, __sk_common.skc_dport);
    dip6->port = swap16(port);
    dip6->size = sip6->size = 20;
...
static int cmd_is_allowed(struct task_struct *task)
{
    ...
    /* locate tid cache in map: tid_cache */
...
struct smith_ipu {
    struct ipaddr_ud sip;
    struct ipaddr_ud dip;
};

static __noinline int query_ipu(struct sock *sk, struct smith_ipu *ip)
{
    ip->sip.size = ip->dip.size = 0;
    ip->sip.family = ip->dip.family = sock_family(sk);
    if (ip->dip.family == AF_INET6 /* ipv6 */) {
        return query_ipv6(sk, &ip->sip, &ip->dip);
    } else if (ip->dip.family == AF_INET /* ipv4 */) {
        return query_ipv4(sk, &ip->sip, &ip->dip);
    }
    return -2;
}
...
static __noinline int process_dns_record(struct var_dns *dns, char *data, unsigned int len)
{
    int rc = 0;

    if (query_ipu(dns->sk, &dns->ip))
        goto out;

    /* we only care port 53 or 5353 */
    if (dns->ip.sip.port != 53 && dns->ip.sip.port != 5353 &&
        dns->ip.dip.port != 53 && dns->ip.dip.port != 5353)
        goto out;
...
static __noinline int process_dns_request(void *ctx, struct sock *sk, char *data, unsigned int len)
{
    ...
    if (rc <= 0)
        goto out;
    dns_print(ctx, &dns->ip.sip, &dns->ip.dip, dns->opcode, dns->rcode, dns->name, rc, dns->type);
...
static __noinline unsigned int append_kstr(char *s, unsigned int off, int max, void *d, unsigned int l, char sep)
{
    unsigned int rc;
    ...
    if (off + l > max)
        l = max - off;
    rc = bpf_probe_read_kernel_str(&s[off & SD_STR_MASK], SD_STR_MASK, d);
    if (rc <= 0)
        return 0;
    if (off > 0)
        s[(off - 1) & SD_STR_MASK] = sep;
    return l;
}

static __noinline unsigned int append_ustr(char *s, unsigned int off, int max, void *d, char sep)
{
    unsigned int rc, l;
    ...
    if (off >= max)
        return 0;
    l = max - off;
    rc = bpf_probe_read_user_str(&s[off & SD_STR_MASK], SD_STR_MASK, d);
    if (rc <= 0)
        return 0;
    if (off > 0)
        s[(off - 1) & SD_STR_MASK] = sep;
...
#   pragma unroll
    ...
    if (len > 0)
        len = bpf_probe_read_str(tid->cmd, len & CMDLINE_MASK, swap->args);
...
static __noinline int prepend_path(char *path, uint32_t *len, char *entry, unsigned int num)
{
...
#   pragma unroll
    ...
    for (int i = 0; i < MAX_PATH_ENTS; i++) {
        struct dentry *root = (void *)READ_KERN(vfsmnt, mnt_root);
        struct dentry *parent = (void *)READ_KERN(dentry, d_parent);
...
static inline char *d_fp_path(struct file *fp, char *path_dat, char *swap, uint32_t *len)
{
...
static __noinline char *d_fd_path(struct task_struct *task, int fd, char *path_dat, char *swap, uint32_t *len)
{
    struct file *fp = fget_raw(task, fd);

    if (!fp)
        return NULL;
    return d_fp_path(fp, path_dat, swap, len);
}

static __always_inline int d_tid_path(struct task_struct *task, struct proc_tid *tid)
{
    struct var_swap {
    ...
    exe_path = d_path(swap->cmds, swap->swap, &exe, &exe_len);
    tid->exe_len = (uint16_t)exe_len;
    bpf_probe_read(tid->exe, (tid->exe_len + 1) & SD_STR_MASK, exe_path);
...
{
    d_tid_path(task, tid); /* WARNING: will destroy local map: sd_get_local */
...
#ifndef TASK_HAS_PID_LINK /* >= 4.19.0 or has back-patched */
/* task_struct->thread_pid introduced from 4.19 */

static __noinline struct pid_namespace *query_ns(struct task_struct *task)
{
    unsigned int level;

    level = READ_KERN(task, thread_pid, level);
    return (void *)READ_KERN(task, thread_pid, numbers[level].ns);
}
...
static __noinline pid_t query_sid(struct task_struct *task)
{
    ...
    ns = query_ns((struct task_struct *)bpf_get_current_task());
...
static __noinline struct pid_namespace *query_ns(struct task_struct *task)
{
    ...
    pid = (void *)READ_KERN(task, pids[PIDTYPE_PID].pid);
    if (!pid)
        goto out;

    /* ns = ns_of_pid(pid) */
    level = READ_KERN(pid, level);
    ns = (void *)READ_KERN(pid, numbers[level].ns);
...
static __noinline pid_t query_sid(struct task_struct *task)
{
    ...
    pid = (void *)READ_KERN(task, singal, pids[PIDTYPE_SID].pid);
    if (!pid)
        goto out;
...
static __noinline pid_t query_pgid(struct task_struct *task)
{
    ...
    /* sid = task_pgrp(task) */
    pid = (void *)READ_KERN(task, group_leader, pids[PIDTYPE_PGID].pid);
    if (!pid)
        goto out;
...
static __noinline struct proc_tid *construct_tid(struct task_struct *task, int fork)
{
    ...
    /* reserve cache from tid_cache for this task */
...
struct var_chmod {
    char path[PATH_NAME_LEN + 4];
    char tmp[PATH_NAME_LEN + 4];
    char dat[SD_STR_MAX];
    uint32_t len;
};

static __noinline void sysret_chmod_comm(void *ctx, struct var_chmod *chmod, int mode, int ret)
{
    chmod_print(ctx, chmod->path, chmod->len, NULL /* fsid */, mode, ret);
}

static __noinline int sysret_chmod(void *ctx, char *fn, u32 mode, int ret)
{
    struct task_struct *task = (void *)bpf_get_current_task();
    struct var_chmod *chmod = NULL;
    char *path;
    int rc;

    chmod = sd_get_local(sizeof(*chmod));
    if (!chmod)
        goto errout;

    rc = bpf_probe_read_user_str(chmod->path, PATH_NAME_LEN, fn);
    if (rc > 0 && chmod->path[0] == '/') {
        chmod->len = rc;
    } else {
        /* build path for current location (pwd) */
        struct path pwd;
        pwd.mnt = (void *)READ_KERN(task, fs, pwd.mnt);
        pwd.dentry = (void *)READ_KERN(task, fs, pwd.dentry);
        chmod->len = 0;
        path = d_path(chmod->dat, chmod->tmp, &pwd, &chmod->len);
        if (!path)
            goto errout;
        rc = bpf_probe_read_str(chmod->path, PATH_NAME_LEN, path);
        if (rc <= 0)
            goto errout;
        rc = append_ustr(chmod->path, chmod->len, PATH_NAME_LEN, fn, '/');
        if (rc)
            chmod->len += rc;
    }
    sysret_chmod_comm(ctx, chmod, mode, ret);
...
static __noinline int sysret_fchmod(void *ctx, int fd, int mode, int ret)
{
    struct task_struct *task = (void *)bpf_get_current_task();
    struct var_chmod *chmod = NULL;
    struct file *filp;
    ...
    filp = fget_raw(task, fd);
    if (!filp)
        goto errout;

    chmod->len = 0;
    path = d_fp_path(filp, chmod->dat, chmod->tmp, &chmod->len);
    if (!path)
        goto errout;
    rc = bpf_probe_read_str(chmod->path, PATH_NAME_LEN, path);
    if (rc <= 0)
...
static __noinline int sysret_fchmodat(void *ctx, int dfd, char *fn, int mode, int ret)
{
    struct task_struct *task = (void *)bpf_get_current_task();
    struct var_chmod *chmod = NULL;
    struct file *filp;
...
static __noinline int sysret_connect(void *ctx, int fd, int ret)
{
    ...
    if (!query_ipu(sk, &ip))
        connect_print(ctx, &ip.dip, &ip.sip, ret);
...
static __noinline int sysret_accept(void *ctx, int fd)
{
    ...
    if (!query_ipu(sk, &ip))
        accept_print(ctx,  &ip.dip,  &ip.sip, fd);
...
static char g_md5_none[32] = "-1";

static __noinline uint64_t query_exe_imgsz(struct task_struct *task)
{
    struct dentry *de;

    de = (struct dentry *)READ_KERN(task, mm, exe_file, f_path.dentry);
    return READ_KERN(de, d_inode, i_size);
}
...
static __noinline int sysret_exec(void *ctx, int ret)
{
    ...
    if (exec->sk) {
        query_ipu(exec->sk, &exec->ip);
    } else {
        exec->ip.dip.size = exec->ip.sip.size = 0;
    }
    ...
    /* query file size of executable image */
    exec->size = query_exe_imgsz(task);
    ...
    execve_print(ctx, exec->pwd, exec->pwd_len, exec->input, exec->in_len,
                 exec->output, exec->out_len, &exec->ip.dip, &exec->ip.sip,
                 exec->pid, exec->tty, exec->tty_len, exec->ssh, exec->ssh_len,
                 exec->ld, exec->ld_len, exec->lib, exec->lib_len, exec->ret,
                 exec->size, g_md5_none);
    sd_put_local(exec);
...
static inline int compat_sysret_is_audit(int id)
{
    int rc = 0;
...
#define NR_chmod            (0x5a) /* 90 */
#define NR_fchmod           (0x5b) /* 91 */
#define NR_fchmodat         (0x10c) /* 268 */
#define NR_fchmodat2        (0x16b)
...
static __noinline int compat_sysret_is_audit(int id)
{
    int rc = 0;
...
#define NR_read             (63)
#define NR_wirte            (64)
#define NR_open             (56) /* openat */

#define NR_mmap             (222)
#define NR_mprotect         (227)
...
#define NR_epoll_wait       ()
#define NR_epoll_ctl        ()
#define NR_epoll_pwait      ()
...
static __noinline int sysret_exit(struct bpf_raw_tracepoint_args *ctx)
{
    ...
    case NR_fchmodat:
        if  (SC_REGS_PARM1(regs) == -100 /* AT_FDCWD */)
            rc = sysret_chmod(ctx, (char *)SC_REGS_PARM2(regs), SC_REGS_PARM3(regs), ret);
        else
            rc = sysret_fchmodat(ctx, SC_REGS_PARM1(regs), (char *)SC_REGS_PARM2(regs),
                                 SC_REGS_PARM3(regs), ret);
        break;
...
static inline int sysret_is_audit(int id)
{
    int rc = 0;
...
static __noinline int sysret_is_critical(int id)
{
    int rc = 0;
...
int tp__proc_exec(struct bpf_raw_tracepoint_args *ctx)
{
    ...
    /* refresh proc_tid cache for newly created task */
    if (tgid == pid) {
        struct proc_tid *tid;
    ...
    /*
     * only trace the successful executions, to be improved, since we
     * need avoid rawtracepoint (step by step) for performance issues.
     */
    if (!cmd_is_allowed(task))
        return sysret_exec(ctx, 0);
    return 0;
}
...
int tp__sys_exit(struct bpf_raw_tracepoint_args *ctx)
{
    ...
    if (!compat) {
        ...
        if (!compat_sysret_is_audit(id))
            return 0;
    }

    if (compat) {
        // bpf_tail_call(ctx, &sysret_progs, 2);
        compat_sysret_exit(ctx);
    } else {
        // bpf_tail_call(ctx, &sysret_progs, 3);
        sysret_exit(ctx);
    }
...
int kp__inode_rename(struct pt_regs *regs)
{
    ...
    file_creation_print(regs, rename->new, len2);
...
#   pragma unroll
    ...
    for (i = 0; i < CMD_ARGS_MAX; i++) {
        ...
        if (rc <= 0)
            break;
        rc = append_kstr(umh->args, len, SD_STR_MAX, umh->swap, rc, ' ');
        if (!rc)
            break;
        len += rc;
    }
...
static __inline int task_prctl(void *ctx, long option, char *name_ptr)
{
    struct var_prctl {
        char name[PATH_NAME_LEN];
    } *prctl;
...
int kp__task_prctl(struct pt_regs *regs)
{
    struct task_struct *task = (void *)bpf_get_current_task();
    ...
    task_prctl(regs, FC_REGS_PARM1(regs), (char *)FC_REGS_PARM2(regs));
    return 0;
}
