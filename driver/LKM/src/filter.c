...
void add_file_notify_checklist(u8 *uuid, unsigned long inode, const char *name, int nlen, int mask)
{
    ...
    node = file_notify_search_rb(&file_notify_checklist, inode);
    if(!node) {
        node = smith_kzalloc(sizeof(struct file_notify_node), GFP_ATOMIC);
        if (!node)
            goto errorout;
        node->data = smith_kzalloc(file_notify_checklist_limit, GFP_ATOMIC);
        if (!node->data) {
            smith_kfree(node);
            goto errorout;
        }
...
static int add_execve_exe_allowlist(char *data)
{
    ...
    node = smith_kzalloc(sizeof(struct allowlist_node), GFP_ATOMIC);
    if (!node)
        return -ENOMEM;
    node->data = data;
...
int filter_init(void)
{
    ...
    sh_mem = smith_kzalloc(SHMEM_MAX_SIZE, GFP_ATOMIC);
