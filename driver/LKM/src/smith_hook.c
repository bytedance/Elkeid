...
static struct files_struct *smith_get_files_struct(struct task_struct *task)
{
    ...
    task_lock(task);
    files = task->files;
    if (files && atomic_add_unless(&files->count, 1, 0)) {
        task_unlock(task);
        return files;
    }
    task_unlock(task);
                        ...
                        pid_t * socket_pid, int *sa_family)
{
    ...
    while (task && task->pid != 1) {
        ...
        for (i = 0; i < files_fdtable(files)->max_fds; i++) {
            ...
            if (!fd_is_open(i, files_fdtable(files))) {
                spin_unlock(&files->file_lock);
                continue;
            }
...
#define SMITH_UDP_THRESHOLD   (6000) /* UDP threshold */
#define SMITH_DNS_KRP_INTERVAL  (10) /* 10 seconds */
#define SMITH_DNS_NET_INTERVAL  (60) /* 60 seconds, must < 500 otherwise may overflow atomic_t */
...
static int smith_dns_work_handler(void *argu)
{
    ...
    g_dns_v4_switch.anchor = rs_get_seconds();
#if IS_ENABLED(CONFIG_IPV6)
    g_dns_v6_switch.anchor = rs_get_seconds();
#endif
...
{
    g_dns_worker = kthread_create(smith_dns_work_handler, 0, "DNS kretprobe");
...
static int __init smith_init(void)
{
    ...
    /* initialize ring slot for message logging */
    ret = rs_init_ring(RING_MODE_FLEX, 128 * 1024);
    if (ret) {
        filter_cleanup();
        return ret;
    }
...
static void smith_exit(void)
{
    ...
    /* finalizing ring slot after all hooks are cleaned */
    rs_fini_ring();
