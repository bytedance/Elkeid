...
## 关于 Elkeid Agent
Elkeid Agent 是一个用户态的程序，主要是用来转发其他功能插件发送来的数据以及通过配置来控制其他插件。
...
在[driver](driver/) 与 [journal_watcher](journal_watcher/)下你可以看到两个示例插件。前者用来解析并丰富Elkeid Driver从内核发来的数据，后者用来监控系统日志。
...
插件的相关信息。
## 数据输出
当前版本的Elkeid Agent更多是用于本地的测试，它不支持远程控制与配置，但是支持将数据发送到远端(通过sarama/kafka)。
