...
func main() {
	...
	zap.S().Info("hostname:", host.Name)
	zap.S().Infof("intranet_ipv4:%v", host.PrivateIPv4)
	zap.S().Infof("intranet_ipv6:%v", host.PrivateIPv6)
	zap.S().Infof("extranet_ipv4:%v", host.PublicIPv4)
	zap.S().Infof("extranet_ipv6:%v", host.PublicIPv6)
